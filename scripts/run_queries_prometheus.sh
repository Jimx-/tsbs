...
EXE_FILE_NAME=${EXE_FILE_NAME:-$(which tsbs_run_queries_prometheus)}
if [[ -z "$EXE_FILE_NAME" ]]; then
    echo "tsbs_run_queries_prometheus not available. It is not specified explicitly and not found in \$PATH"
    exit 1
fi
...
if [ "$#" -gt 0 ]; then
    ...
    echo "Do not have any files specified - run from default queries folder as ${BULK_DATA_DIR}/queries_prometheus*"
    for FULL_DATA_FILE_NAME in "${BULK_DATA_DIR}/queries_prometheus"*; do
        run_file $FULL_DATA_FILE_NAME
    done
fi
